...
export function SearchFilterBar({
  ...
  const hasAnyFilterActive = categoryArray.length > 0 || locationArray.length > 0 || priceArray.length > 0 || ratingArray.length > 0 || availabilityArray.length > 0 || openNow

  return (
    <div className="flex items-center overflow-x-auto invisible-scrollbar w-full py-1">
      <div className="flex gap-2 w-max items-center pb-2">
        ...
        <button 
          type="button"
          className="relative flex items-center shrink-0 h-10 px-5 ml-1 rounded-full border border-gray-900 bg-gray-900 text-white text-sm font-medium transition-opacity hover:opacity-90 active:scale-95"
        >
        ...
        {hasAnyFilterActive && (
          <button 
            type="button"
            onClick={handleClear}
            className="flex items-center text-xs font-semibold text-gray-400 hover:text-red-500 transition-colors ml-4 uppercase tracking-wider h-10"
          >
            Clear
          </button>
        )}
