...
export class BusinessService {
  async createBusiness(data: BusinessServiceCreateData): Promise<Business> {
    ...
    // Find or create user using the provided userEmail
    const user = await userRepository.findOrCreate({ email: data.userEmail });

    // Create business - exclude the runtime-only `userEmail` property before saving
