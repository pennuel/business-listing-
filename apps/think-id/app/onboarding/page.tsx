...
import { Progress } from "@think-id/ui/components/ui/progress";
...
export default function OnboardingPage() {
  ...
  // If editing (businessId provided), fetch that business data.
  useEffect(() => {
    ...
    const fetchExisting = async () => {
      try {
        const res = await fetch(`/api/businesses?id=${encodeURIComponent(businessId)}`);
        if (!res.ok) return;
        ...
        const mapped = {
          ...
          id: b.id,
          coordinates: (b.latitude && b.longitude) ? { lat: b.latitude, lng: b.longitude } : undefined,
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          ...
          if (isEditing && businessData.id) {
            ...
            router.push("/?section=businesses");
          }
        }, 1500);
      } else {
