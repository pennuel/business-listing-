...
export interface BusinessServiceCreateData extends Omit<CreateBusinessData, "userId"> {
  userEmail: string
}
...
export class BusinessService {
  async createBusiness(data: BusinessServiceCreateData): Promise<Business> {
    ...
    // Find or create user
    const user = await userRepository.findOrCreate(data.userEmail)
  ...
  async getBusinessesByLocation(county: string, subCounty?: string): Promise<Business[]> {
  ...
  async updatePaymentStatus(id: string, paymentStatus: string): Promise<Business> {
