...
import { ApplicationGroupBillingPage } from "./components/application-group-billing-page"
import { ApplicationBillingManagementPage } from "./components/application-billing-management-page"
...
export default function ProfileDashboard() {
  ...
  const [selectedBillingGroup, setSelectedBillingGroup] = useState<string | null>(null)
  const [selectedBillingApp, setSelectedBillingApp] = useState<string | null>(null)
  ...
  const renderContent = () => {
    ...
    if (selectedApplication) {
      return <ApplicationDetailPage appKey={selectedApplication} onBack={() => setSelectedApplication(null)} />
    }

    switch (activeSection) {
      case "dashboard":
        return <DashboardContent onApplicationsSelect={handleApplicationsSelect} />
      case "profile":
        return (
          <ProfilePage onEditProfile={() => setIsEditingProfile(true)} onBack={() => setActiveSection("dashboard")} />
        )
      case "applications":
        return (
          <ApplicationsPage
      ...
      case "billing":
        if (selectedBillingApp) {
          return (
            <ApplicationBillingManagementPage appKey={selectedBillingApp} onBack={() => setSelectedBillingApp(null)} />
          )
        }
        ...
        return <BillingPage onApplicationGroupSelect={setSelectedBillingGroup} />
      case "account":
        return <AccountSettingsPage onBack={() => setActiveSection("dashboard")} />
      case "activity":
        return (
          <div className="p-4">
      ...
      default:
        ...
        if (activeSection.startsWith("app-")) {
          return <ApplicationDetailPage appKey={activeSection} onBack={() => setActiveSection("applications")} />
        }
        return <DashboardContent onApplicationsSelect={handleApplicationsSelect} />
    }
  }
    ...
    switch (activeSection) {
      ...
      case "applications":
        return selectedCategory
          ? `Applications - ${selectedCategory.charAt(0).toUpperCase() + selectedCategory.slice(1)}`
          : "Applications"
      case "security":
        return "Security"
      case "billing":
